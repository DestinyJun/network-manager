...
export class WellModifyComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.sensorsFormsBody = {
      ...
      conduitId: ['', [UniversalValidators.maxLength(20)]],
      dataCollectorId: ['', [Validators.required, UniversalValidators.maxLength(12)]],
    };
    this.sensorsFormBodyHtml = [
      // new TextBox('井ID', 'initialManholeId', [new ValidMsg',, ' 'text', '', ''),
      new TextBox('传感器所属模式', 'sensormode', [new ValidMsg('required', '必填项'), new ValidMsg('maxLength', '长度不能超过5位的整数')], 'text', '', ''),
      new TextBox('模块ID', 'modeId', [new ValidMsg('required', '必填项'), new ValidMsg('maxLength', '长度不能超过5位的整数')], 'text', '', ''),
      new TextBox('高度', 'hight', [new ValidMsg('required', '必填项'), new ValidMsg('maxLength', '长度不能超过20位')], 'text', '', ''),
      new TextBox('传感器在模块中的位置', 'modePlace', [new ValidMsg('required', '必填项'), new ValidMsg('maxLength', '长度为1位')], 'text', '', ''),
      new TextBox('导管ID', 'conduitId', [new ValidMsg('required', '必填项'), new ValidMsg('maxLength', '长度不能超过20位')], 'text', '', ''),
      new TextBox('数据收集器ID', 'dataCollectorId', [new ValidMsg('required', '必填项'), new ValidMsg('maxLength', '长度不能超过12位')], 'text', '', '')
    ];
    // 井ID保持一致
